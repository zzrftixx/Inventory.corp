                ...
                <div class="flex-1">
                    ...
                    <select id="item_selector" class="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none bg-white">
                        ...
                        @foreach($items as $item)
                            <option value="{{ $item->id }}" data-kode="{{ $item->kode_barang }}" data-nama="{{ $item->nama_barang }}" data-satuan="{{ $item->satuan }}" data-stok="{{ $item->stok_saat_ini }}" data-min="{{ $item->batas_stok_minimum }}">
                ...
                <table class="w-full text-left border-collapse" id="poTable">
                    <thead>
                        <tr class="bg-slate-100 text-slate-600">
                            ...
                            <th class="py-3 px-4 font-medium text-sm text-center">Stok Saat Ini</th>
                            <th class="py-3 px-4 font-medium text-sm text-center">Batas Min.</th>
                            <th class="py-3 px-4 font-medium text-sm w-40 text-center">Qty Dipesan <span class="text-red-500">*</span></th>
                    ...
                    <tbody id="items-container" class="divide-y divide-slate-100">
                        @if(request()->has('auto') && count($autoItems) > 0)
                            ...
                            <tr id="row-{{ $auto->id }}" class="bg-red-50">
                                ...
                                <td class="py-3 px-4 text-sm text-center font-bold text-red-600">{{ $auto->stok_saat_ini }}</td>
                                <td class="py-3 px-4 text-sm text-center text-slate-500">{{ $auto->batas_stok_minimum }}</td>
                                <td class="py-3 px-4 text-center">
                                    <div class="flex items-center justify-center">
                                        <!-- Recommendation: order amount to reach twice the minimum limit -->
                                        @php $recOrder = ($auto->batas_stok_minimum * 2) - $auto->stok_saat_ini; $recOrder = $recOrder < 1 ? 10 : $recOrder; @endphp
                                        <input type="number" name="items[{{ $auto->id }}][qty_butuh]" value="{{ $recOrder }}" min="1" class="w-16 px-2 py-1 text-sm border border-red-300 rounded focus:outline-none focus:border-red-500 text-center">
                        ...
                        @else
                            <tr id="empty-row">
                                <td colspan="6" class="py-6 text-center text-slate-400 text-sm">List pesanan masih kosong. Silakan tambah barang di atas.</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
...
<script>
    document.getElementById('btn-add-item').addEventListener('click', function() {
        ...
        tr.innerHTML = `
            ...
            <td class="py-3 px-4 text-sm text-center font-bold ${stokColor}">${stok}</td>
            <td class="py-3 px-4 text-sm text-center text-slate-500">${min}</td>
            <td class="py-3 px-4 text-center">
                <div class="flex items-center justify-center">
                    <input type="number" name="items[${id}][qty_butuh]" value="10" min="1" class="w-16 px-2 py-1 text-sm border border-slate-300 rounded focus:outline-none focus:border-primary text-center">
