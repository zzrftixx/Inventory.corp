    ...
    <div class="w-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mt-6">
        ...
        <form action="{{ route('purchase-orders.store') }}" method="POST" id="poForm">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50">
                <div>
                    <label for="supplier_id" class="block text-sm font-medium text-slate-700 mb-1">Pabrik / Supplier <span
                ...
                <div>
                    <label for="tanggal_po" class="block text-sm font-medium text-slate-700 mb-1">Tanggal Pemesanan <span
                            class="text-red-500">*</span></label>
                    <input type="date" name="tanggal_po" id="tanggal_po" value="{{ date('Y-m-d') }}" required
            ...
            <div class="p-6 border-t border-slate-100">
                ...
                <div class="mb-6 bg-slate-50 p-4 rounded-lg border border-slate-200">
                    ...
                    <p class="text-xs text-slate-500 mt-2"><i class="ph ph-info font-bold"></i> Ketik nama atau kode barang
                        (minimal 1 huruf). Cth: Kusen, ALX, Kaca.</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse" id="poTable">
                        <thead>
                            <tr class="bg-slate-100 text-slate-600">
                                ...
                                <th class="py-3 px-4 font-medium text-sm w-32 text-center">Qty <span
                                        class="text-red-500">*</span></th>
                                <th class="py-3 px-4 font-medium text-sm w-40 text-right">Harga Beli Sat. <span
                                        class="text-red-500">*</span></th>
                        ...
                        <tbody id="items-container" class="divide-y divide-slate-100">
                                ...
                                @foreach($autoItems as $auto)
                                    <tr id="row-{{ $auto->id }}" class="bg-red-50">
                                        ...
                                        <td class="py-3 px-4 text-sm text-center font-bold text-red-600">
                                            {{ $auto->stok_saat_ini }}
                                            <span class="block text-[10px] text-slate-500 font-normal">Min:
                                                {{ $auto->batas_stok_minimum }}</span>
                                        </td>
                                        <td class="py-3 px-4 text-center">
                                            <div class="flex items-center justify-center">
                                                @php $recOrder = ($auto->batas_stok_minimum * 2) - $auto->stok_saat_ini;
                                                $recOrder = $recOrder < 1 ? 10 : $recOrder; @endphp
                                                <input type="number" name="items[{{ $auto->id }}][qty_butuh]"
                                                    id="qty-{{ $auto->id }}" value="{{ $recOrder }}" min="1"
                                                    class="w-16 px-2 py-1 text-sm border border-red-300 rounded focus:outline-none focus:border-red-500 text-center"
                                                    onchange="calcRow({{ $auto->id }})" onkeyup="calcRow({{ $auto->id }})">
                                                <span class="ml-2 text-xs text-slate-500">/{{ $auto->satuan }}</span>
                                            </div>
                                        </td>
                                        <td class="py-3 px-4 text-right">
                                            <input type="text" inputmode="numeric" name="items[{{ $auto->id }}][harga_beli_satuan]"
                                                id="harga-{{ $auto->id }}" value="{{ $auto->harga_beli_rata_rata }}"
                                                class="input-rupiah w-full px-2 py-1 text-sm border border-slate-300 rounded focus:outline-none focus:border-primary text-right"
                                                oninput="formatRupiah(this); calcRow({{ $auto->id }})">
                                        </td>
                                        ...
                                        <td class="py-3 px-4 text-center">
                                            <button type="button" onclick="removeRow({{ $auto->id }})"
                            ...
                            @else
                                <tr id="empty-row">
                                    <td colspan="7" class="py-6 text-center text-slate-400 text-sm">List pesanan masih kosong.
                                        Silakan tambah barang di atas.</td>
                                </tr>
                            @endif
                        </tbody>
                        <tfoot class="bg-slate-50 border-t border-slate-200">
                            <tr>
                                <td colspan="5" class="py-4 px-4 text-right font-bold text-slate-700">Estimasi Total PO:
                                </td>
            ...
            <div class="flex justify-end p-6 border-t border-slate-100 bg-slate-50 space-x-3">
                ...
                <button type="button" onclick="validateAndSubmit()"
                    class="px-6 py-2.5 text-sm font-bold text-white bg-primary hover:bg-sky-600 rounded-lg transition-colors shadow-sm">
                    ...
                    fetch("{{ route('purchase-orders.store') }}", {
                        ...
                        .then(async response => {
                            if (!response.ok) {
                                ...
                                // Handle 500 explicit errors
                                Swal.fire('Error System', data.message || 'Terjadi kesalahan pada sistem backend.', 'error');
                        ...
                        .catch(error => {
                            if (error.message !== 'Validation Failed' && error.message !== 'Server Error') {
