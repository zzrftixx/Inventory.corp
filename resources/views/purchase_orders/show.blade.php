        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 col-span-1 flex flex-col justify-between">
            ...
            <div class="mt-6 pt-4 border-t border-slate-100">
                <a href="{{ route('purchase-orders.print', $purchaseOrder->id) }}" target="_blank"
                    class="w-full justify-center bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center shadow-sm">
                            ...
                            <td class="py-3 px-6 text-sm font-medium text-slate-800">
                                ...
                                @if($detail->metadata_kalkulasi)
                                    ...
                                    <div class="text-red-500 text-xs mt-1 p-1 bg-red-50 rounded border border-red-100 inline-block">
                                        ...
                                        Harga Dasar PO: Rp {{ number_format($meta->modal_beku, 0, ',', '.') }} <br>
    ...
    @if($purchaseOrder->status == 'Ordered')
        <div class="flex justify-end mt-4">
            <form action="{{ route('purchase-orders.destroy', $purchaseOrder->id) }}" method="POST"
                onsubmit="return confirm('Apakah Anda yakin ingin DIBATALKAN/HAPUS pesanan ini? Aksi ini tidak dapat dikembalikan.');">
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="text-slate-400 hover:text-red-600 px-4 py-2 text-sm font-medium transition-colors flex items-center">
